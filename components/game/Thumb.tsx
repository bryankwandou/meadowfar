...
import { cosmeticThumb, itemThumb } from "@/lib/game/thumbs";
import type { HeroId } from "@/lib/progression";

// Rendered 3D icon for a wardrobe piece or an inventory item.
export default function Thumb({ kind, id, hero, alt, className = "h-16 w-16" }: { kind: "cosmetic" | "item"; id: string; hero?: HeroId; alt: string; className?: string }) {
  const [url, setUrl] = useState("");
  useEffect(() => {
    let live = true;
    const done = (u: string) => queueMicrotask(() => live && setUrl(u));
    if (kind === "cosmetic") cosmeticThumb(hero ?? "girl", id, done);
